...
public class ResidenceListingsDriver {
    public static void main(String[] args) {
        ...
        String[] doneNextButtons = {"Next", "Done"};
        ...
        do {
            ...
            if (mainMenuSelection == null) {
                ...
                if (residenceType.equals(residenceTypeFindSelections[0])) {
                    ...
                    int result = 0;
                    for (int i = 0; i < rl.getResidences().length; i++) {
                            if (rl.getResidences()[i] instanceof Apartment && rl.getResidences()[i] != null) {
                                result = JOptionPane.showOptionDialog(null, rl.getResidences()[i].toString(), "Find a Listing", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE, null, doneNextButtons, null);
                            }
                    ...
                    int result = 0;
                    for (int i = 0; i < rl.getResidences().length; i++) {
                        if (rl.getResidences()[i] instanceof House && rl.getResidences()[i] != null) {
                            result = JOptionPane.showOptionDialog(null, rl.getResidences()[i].toString(), "Find a Listing", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE, null, doneNextButtons, null);
                        }
                        if (result == JOptionPane.NO_OPTION) {
                            break;
                    ...
                    int result = 0;
                    for (int i = 0; i < rl.getResidences().length; i++) {
                        if ((rl.getResidences()[i] instanceof House || rl.getResidences()[i] instanceof Apartment) && rl.getResidences()[i] != null) {
                            result = JOptionPane.showOptionDialog(null, rl.getResidences()[i].toString(), "Find a Listing", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE, null, doneNextButtons, null);
                        }
                        if (result == JOptionPane.NO_OPTION) {
                            break;
